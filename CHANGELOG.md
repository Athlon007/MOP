...
- Stolen machine slot is now disabled
- Human NPCs now despawn
...
- Added experimental save file optimization (disabled by default)
  ...
  - Reduces the load time (especially for older saves)
  ...
  - WARNING: This is a highly experimental version, I recommend making your own backup of entire save folder! **USE ONLY AT YOUR OWN RISK!**

### Changes

- (My Summer Car) Restocking now works no matter the player the player's distance to the store (previously, if player was too close to the store, the restocking script wouldn't trigger)
...
- (My Summer Car Bug) Fixed save files staying in "read only" state, if the game crashed when trying to switch from the game scene to main menu after saving, causing saving to not work properly
